...
//        behavior.translatesReferenceBoundsIntoBoundary = true
    ...
    private var collisonHandlers = [String:()->Void]()
    ...
    func setBoundary(_ path: UIBezierPath?, named name: String, handler: (()->Void)?){
        ...
        collisonHandlers[name] = nil
        if path != nil {
            collider.addBoundary(withIdentifier: name as NSString, for: path!)
            collisonHandlers[name] = handler
        }
        
    }
    
    func collisionBehavior(_ behavior: UICollisionBehavior, beganContactFor item: UIDynamicItem, withBoundaryIdentifier identifier: NSCopying?, at p: CGPoint) {
        if let name = identifier as? String, let handler = collisonHandlers[name]{
            handler()
        }
    }
    ...
    private func startRecapturingWaywardAsteroids() {
        if recaptureTimer == nil{
            recaptureTimer = Timer.scheduledTimer(withTimeInterval: 0.5, repeats: true, block: {[weak self] timer in
                for asteroid in self?.asteroids ?? [] {
